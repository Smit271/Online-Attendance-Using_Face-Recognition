...
def write_text(frame, data, dims):
	'''
    write label name on face
    '''
	startX, startY, endX, endY,h = dims
	text = str(data["class_name"]) + "-" + str(round(data["class_probability"] * 100, 2)) + "%"
	text_face = "f= " + str(round(data["detection_conf"], 2))
...
def camera():
	...
	print("timetable id:",timetable_id)
	...
	if today_data:
		for each in today_data.all():
			...
			# print("student mail:",student_email)
	...
	while True:
		...
		for i in range(0, detections.shape[2]):
			...
			try:
				...
				if class_name not in attendance_marked:
					try:
					...
					if attendance_started[class_name]["count"] >= frame_count_threshold:
						...
						attendance_marked[class_name] = [true_count,
														 round(attendance_started[class_name]["confidence"] / true_count,
															   2)]
				# print(attendance_started)
				# print(frame_count)
				data = {"class_name":class_name,"class_probability":class_probability,"detection_conf":confidence}
				write_text(frame, class_name, dims.append(h))
			except Exception:
				# traceback.print_exc()
				# print("title outside")
				pass
